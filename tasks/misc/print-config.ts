...
import { eSmartBCHNetwork, eNetwork } from '../../helpers/types';

task('print-config', 'Inits the DRE, to have access to all the plugins')
  .addParam('dataProvider', 'Address of BandzProtocolDataProvider')
  ...
  .setAction(async ({ pool, dataProvider }, localBRE) => {
    ...
    const protocolDataProvider = await getBandzProtocolDataProvider(dataProvider);
