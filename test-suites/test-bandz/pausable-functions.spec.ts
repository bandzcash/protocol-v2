...
makeSuite('Pausable Pool', (testEnv: TestEnv) => {
  ...
  it('Withdraw', async () => {
    ...
    await expect(
      pool
        .connect(users[0].signer)
        .withdraw(flexUsd.address, amountFlexUSDtoDeposit, users[0].address)
    ).to.revertedWith(LP_IS_PAUSED);
  ...
  it('Borrow', async () => {
    ...
    await expect(
      pool.connect(user.signer).borrow(flexUsd.address, '1', '1', '0', user.address)
  ...
  it('Repay', async () => {
    ...
    // Try to execute liquidation
    await expect(
      pool.connect(user.signer).repay(flexUsd.address, '1', '1', user.address)
    ).revertedWith(LP_IS_PAUSED);
  ...
  it('SwapBorrowRateMode', async () => {
    ...
    await pool
      .connect(user.signer)
      .deposit(flexUsd.address, amountFlexUSDToDeposit, user.address, '0');
