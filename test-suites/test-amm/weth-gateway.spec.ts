...
makeSuite('Use native ETH at LendingPool via WBCHGateway', (testEnv: TestEnv) => {
  const zero = BigNumber.from('0');
  const depositSize = parseEther('5');
  const daiSize = parseEther('10000');
  it('Deposit WBCH via WethGateway and DAI', async () => {
  ...
  it('Borrowing stable WBCH should fail since stable borrowing is disabled', async () => {
  ...
  it('Borrow ETH via delegateApprove ETH and repays back', async () => {
    const { users, wethGateway, aWBCH, weth, helpersContract, pool } = testEnv;
    const borrowSize = parseEther('1');
    const user = users[2];
    const { variableDebtTokenAddress } = await helpersContract.getReserveTokensAddresses(
      weth.address
