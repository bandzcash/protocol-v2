...
makeSuite('Mainnet Check list', (testEnv: TestEnv) => {
  ...
  it('Deposit WBCH', async () => {
    ...
    await wbchGateway.connect(user.signer).depositETH(user.address, '0', { value: depositSize });
  ...
  it('Withdraw WBCH - Partial', async () => {
    ...
    const { gasUsed: withdrawGas } = await waitForTx(
      await wbchGateway.connect(user.signer).withdrawETH(partialWithdraw, user.address)
    );
  ...
  it('Withdraw WBCH - Full', async () => {
    ...
    // Full withdraw
    const { gasUsed: withdrawGas } = await waitForTx(
      await wbchGateway.connect(user.signer).withdrawETH(MAX_UINT_AMOUNT, user.address)
    );
  ...
  it('Borrow BCH via delegateApprove BCH and repays back', async () => {
    const { users, wbchGateway, aWBCH, wbch, helpersContract } = testEnv;
    const borrowSize = parseEther('1');
    const user = users[2];
    const { variableDebtTokenAddress } = await helpersContract.getReserveTokensAddresses(
      wbch.address
    ...
    await waitForTx(await wbchGateway.connect(user.signer).borrowETH(borrowSize, '2', '0'));
  ...
  it('Getters should retrieve correct state', async () => {
    const { aWBCH, wbch, pool, wbchGateway } = testEnv;

    const WETHAddress = await wbchGateway.getWETHAddress();
    const aWETHAddress = await wbchGateway.getAWETHAddress();
    const poolAddress = await wbchGateway.getLendingPoolAddress();

    expect(WETHAddress).to.be.equal(wbch.address);
    expect(aWETHAddress).to.be.equal(aWBCH.address);
